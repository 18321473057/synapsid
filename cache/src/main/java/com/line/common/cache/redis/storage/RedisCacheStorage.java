...
import com.line.common.cache.redis.constant.RedisConstants;
import com.line.common.cache.redis.exception.KeyIsNotFoundException;
import com.line.common.cache.redis.exception.RedisCacheStorageException;
...
public class RedisCacheStorage<K, V> implements IRemoteCacheStore<K, V>, InitializingBean {
     ...
     * @return boolean 是否执行成功
     */
    public boolean set(K key, NullObject nullObject, int exp) {
        if (key == null) {
            throw new RedisCacheStorageException("key does not allow for null!");
        }
        redisTemplate.opsForValue().set(key, nullObject, exp, TimeUnit.SECONDS);
        return true;
    }
