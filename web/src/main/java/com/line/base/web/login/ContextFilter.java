...
public class ContextFilter extends OncePerRequestFilter implements ApplicationContextAware {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (request instanceof HttpServletRequest && response instanceof HttpServletResponse) {
            ...
            String userId = null;
            //判断应用是否接入了CAS统一登录
            boolean useCas = "true".equals(getEnvironment().getProperty("line.cas.client.enable"));
            if (!useCas) {
...
//        PageResponse redirectResponse = new PageResponse();
//        redirectResponse.setErrorCode(PageResponse.ERROR_REDIRECT);
//        //判断应用是否接入了CAS统一登录
